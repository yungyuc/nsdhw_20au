...
struct Matrix {
    ...
    Matrix(size_t nrow, size_t ncol) : m_nrow(nrow), m_ncol(ncol), m_buffer(matrix_allocator) {
    ...
    Matrix(Matrix const &other) : m_nrow(other.m_nrow), m_ncol(other.m_ncol), m_buffer(matrix_allocator) {
    ...
    Matrix(Matrix && other)
      : m_nrow(std::exchange(other.m_nrow, 0)), m_ncol(std::exchange(other.m_ncol, 0)),
    ...
    std::vector<double> buffer_vector() const {
        return std::vector<double>(m_buffer.data(), m_buffer.data() + size());
    }
