...
private:
    vector<double, CustomAllocator<double>> matrix;
    int _nrow, _ncol;
};
...
Matrix::Matrix(int row, int col) : matrix(row * col, 0, custom_allocator) {
    _nrow = row;
    _ncol = col;
}

Matrix::Matrix(const Matrix& other)
    : matrix(other._nrow * other._ncol, 0, custom_allocator) {
    _nrow = other._nrow;
    _ncol = other._ncol;
...
size_t bytes() {
    return custom_allocator.counter.bytes();
}

size_t allocated() {
    return custom_allocator.counter.allocated();
}

size_t deallocated() {
    return custom_allocator.counter.deallocated();
}
